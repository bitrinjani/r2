...
describe('Coincheck BrokerAdapter', () => {
  ...
  test('send StopLimit order', async () => {
    const target = new BrokerAdapterImpl({ config });
    const order = { broker: Broker.Bitflyer, cashMarginType: CashMarginType.Cash, symbol: 'BTCJPY', type: OrderType.StopLimit };
    try {
      await target.send(order);
    } catch (ex) {
      return;
  ...
  test('send wrong time in force', async () => {
    const target = new BrokerAdapterImpl({ config });
    const order = { broker: Broker.Bitflyer, cashMarginType: CashMarginType.Cash, symbol: 'BTCJPY', type: OrderType.Market, timeInForce: 'MOCK' };
    try {
      await target.send(order);
    } catch (ex) {
      return;
  ...
  test('refresh unknown order id', async () => {
    const target = new BrokerAdapterImpl({ config });
    const order = { "symbol": "BTCJPY", "type": "Limit", "timeInForce": "None", "id": "438f7c7b-ed72-4719-935f-477ea043e2b0", "status": "New", "creationTime": "2017-11-03T09:20:06.687Z", "executions": [], "broker": "Bitflyer", "size": 0.01, "side": "Sell", "price": 846700, "cashMarginType": "Cash", "brokerOrderId": "MOCK", "sentTime": "2017-11-03T09:20:07.292Z", "lastUpdated": "2017-11-03T09:20:07.292Z" };
    await target.refresh(order);
    expect(order.status).toBe(OrderStatus.New);
  });
});