...
test('reverse fill', async () => {
  ...
  const buyLeg = new OrderImpl('Dummy1', OrderSide.Buy, 0.1, 100, CashMarginType.Cash, OrderType.Limit, 10);
  buyLeg.filledSize = 0.1;
  buyLeg.status = OrderStatus.Filled;
  const sellLeg = new OrderImpl('Dummy2', OrderSide.Sell, 0.1, 110, CashMarginType.Cash, OrderType.Limit, 10);
